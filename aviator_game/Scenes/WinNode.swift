...
class WinNode: SKSpriteNode {
    ...
    init(color: UIColor, size: CGSize, gainedPointsInGame: Int, timePassed: Int) {
        ...
        let backgroundWin = SKSpriteNode(imageNamed: "win_bg")
        ...
        let newRecordLabel = SKSpriteNode(imageNamed: "new_record")
        newRecordLabel.size = CGSize(width: 280, height: 42)
        newRecordLabel.zPosition = 7
        newRecordLabel.position = CGPoint(x: size.width / 2, y: size.height / 2 + 15)
        addChild(newRecordLabel)
        
        let timePassedBg = SKSpriteNode(imageNamed: "time_win_bg")
